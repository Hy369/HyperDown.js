...
(function() {
  ...
  Parser = (function() {
    ...
    Parser.prototype.parseInline = function(text, whiteList, clearHolders, enableAutoLink) {
      ...
      text = text.replace(/!\[((?:[^\]]|\\\]|\\\[)*?)\]\(((?:[^\)]|\\\)|\\\()+?)\)/g, (function(_this) {
        return function() {
          var escaped, extra, matches, style, styles, url;
          matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
          escaped = htmlspecialchars(_this.escapeBracket(matches[1]));
          url = _this.escapeBracket(matches[2]);
          url = _this.cleanUrl(url);
          extra = _this.parseExtra(url);
          if (extra != null) {
            styles = [];
            if (extra != null ? extra.width : void 0) {
              styles.push("width:" + extra.width + ";");
            }
            if (extra != null ? extra.height : void 0) {
              styles.push("height:" + extra.height + ";");
            }
          }
          style = styles != null ? ' style="' + (styles.join(' ')) + '"' : '';
          return _this.makeHolder("<img src=\"" + url + "\" alt=\"" + escaped + "\" title=\"" + escaped + "\"" + style + ">");
        };
      })(this));
      text = text.replace(/!\[((?:[^\]]|\\\]|\\\[)*?)\]\[((?:[^\]]|\\\]|\\\[)+?)\]/g, (function(_this) {
        return function() {
          var escaped, extra, matches, result, style, styles;
          matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
          escaped = htmlspecialchars(_this.escapeBracket(matches[1]));
          if (_this.definitions[matches[2]] != null) {
            extra = _this.definitions[matches[2]].extra;
            styles = [];
            if (extra != null ? extra.width : void 0) {
              styles.push("width:" + extra.width + ";");
            }
          ...
          style = styles != null ? ' style="' + (styles.join(' ')) + '"' : '';
          result = _this.definitions[matches[2]] != null ? "<img src=\"" + _this.definitions[matches[2]].url + "\" alt=\"" + escaped + "\" title=\"" + escaped + "\"" + style + ">" : escaped;
      ...
      text = text.replace(/\[((?:[^\]]|\\\]|\\\[)+?)\]\[((?:[^\]]|\\\]|\\\[)+?)\]/g, (function(_this) {
        return function() {
          ...
          matches = 1 <= arguments.length ? slice.call(arguments, 0) : [];
          escaped = _this.parseInline(_this.escapeBracket(matches[1]), '', false, false);
          result = _this.definitions[matches[2]] != null ? "<a href=\"" + _this.definitions[matches[2]].url + "\">" + escaped + "</a>" : escaped;
          return _this.makeHolder(result);
        };
      })(this));
    ...
    Parser.prototype.parseBlock = function(text, lines) {
      var align, aligns, block, emptyCount, head, isAfterList, j, key, l, len, len1, len2, line, m, matches, num, ref, row, rows, space, special, tag, url;
      ref = text.split("\n");
      for (j = 0, len = ref.length; j < len; j++) {
        line = ref[j];
        lines.push(line);
        ...
        switch (true) {
          ...
          case !!(matches = line.match(/^\s*\[((?:[^\]]|\]|\[)+?)\]:\s*(.+)$/)):
            url = {
              ...
              extra: this.parseExtra(matches[2])
            };
            this.definitions[matches[1]] = url;
    ...
    Parser.prototype.parseExtra = function(url) {
      var extra, matches;
      extra = null;
      if (!!(matches = url.match(/s=([\dA-Za-z.]*)(?:\*([\dA-Za-z.]*))/i))) {
        extra = {};
        if (!!matches[1]) {
          extra.width = matches[1];
        }
        if (!!matches[2]) {
          extra.height = matches[2];
        }
      }
      return extra;
    };
